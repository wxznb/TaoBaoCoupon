 ...
 * 2. 把所有参数名和参数值串在一起
 * 3. 使用MD5或者是HMAC加密
 * 4. 先将字符串转成二进制，然后再把二进制转化为大写的十六进制(正确签名应该为32大写字符串，此方法需要时使用)
 */
import md5 from "@/static/md5.min.js";

const sign = ( options = {}, secret = "" ) => {
	// 1. 将所有参数进行排序
	let optionsName = Object.keys(options).sort();
	...
	// 3. 使用MD5或者是HMAC加密
	let optionStringMd5 = md5(`${secret}${optionString}${secret}`).toUpperCase();
